...
jQuery(function($) {
    ...
    const modals = {
        ...
        customRange: $('#br-expense-custom-range-filter-modal')
    };
    ...
    wrapper.on('click', '.br-edit-category-btn', function(e) {
        e.preventDefault();
        const id = $(this).data('id');
        $.post(br_expense_ajax.ajax_url, { action: 'br_get_expense_category_details', nonce: br_expense_ajax.nonce, id: id })
            .done(response => {
                if (response.success) {
                    const d = response.data;
                    const form = modals.category.find('form');
                    form.find('#category_id').val(d.id);
                    form.find('#category_name').val(d.name);
                    modals.category.find('#br-category-modal-title').text('Edit Category');
                    openModal(modals.category);
                } else {
                    alert(response.data.message);
                }
    ...
    wrapper.on('click', '.br-delete-expense-btn, .br-delete-monthly-expense-btn, .br-delete-category-btn', function(e) {
        e.preventDefault();
        ...
        } else if ($(this).is('.br-delete-monthly-expense-btn')) {
            action = 'br_delete_monthly_expense';
        } else if ($(this).is('.br-delete-category-btn')) {
            action = 'br_delete_expense_category';
            confirm_message = 'Are you sure? All existing expenses in this category will be moved to "Uncategorized".';
        }
