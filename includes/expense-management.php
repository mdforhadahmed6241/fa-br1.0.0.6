...
function br_expense_category_tab_html() {
    ...
    // The button is now rendered inside the filters row function
...
function br_expense_render_date_filters_html($current_tab = 'expense_list', $list_table_object = null) {
    ...
    <div class="br-filters">
        ...
        <div class="br-expense-actions">
            ...
            <?php elseif ($current_tab === 'expense_category'): ?>
                <button id="br-add-category-btn" class="button button-primary"><?php _e('Add New Category', 'business-report'); ?></button>
            <?php endif; ?>
        </div>
    </div>
    <?php
...
// --- NEW: Get Category Details AJAX ---
function br_ajax_get_expense_category_details() {
    check_ajax_referer('br_expense_nonce', 'nonce');
    if (!current_user_can('manage_woocommerce')) { wp_send_json_error(['message' => 'Permission denied.']); }
    
    global $wpdb;
    $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
    $category = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}br_expense_categories WHERE id = %d", $id));
    if ($category) {
...
add_action('wp_ajax_br_get_expense_category_details', 'br_ajax_get_expense_category_details');

// --- NEW: Delete Category AJAX (Data Migration) ---
function br_ajax_delete_expense_category() {
    check_ajax_referer('br_expense_nonce', 'nonce');
    if (!current_user_can('manage_woocommerce')) { wp_send_json_error(['message' => 'Permission denied.']); }
    ...
    // 1. Get Uncategorized ID
    $uncategorized_id = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}br_expense_categories WHERE name = %s", 'Uncategorized'));
    
    if (!$uncategorized_id) {
        // Create it if missing (safeguard)
        $wpdb->insert($wpdb->prefix . 'br_expense_categories', ['name' => 'Uncategorized']);
        $uncategorized_id = $wpdb->insert_id;
    }

    if ($id == $uncategorized_id) {
        wp_send_json_error(['message' => 'Cannot delete the default Uncategorized category.']);
    }

    // 2. Migrate Regular Expenses
    $wpdb->update(
        $wpdb->prefix . 'br_expenses', 
        ['category_id' => $uncategorized_id], 
        ['category_id' => $id]
    );

    // 3. Migrate Monthly Expenses
    $wpdb->update(
        $wpdb->prefix . 'br_monthly_expenses', 
        ['category_id' => $uncategorized_id], 
        ['category_id' => $id]
    );

    // 4. Delete Category
    $result = $wpdb->delete($wpdb->prefix . 'br_expense_categories', ['id' => $id]);
...
add_action('wp_ajax_br_delete_expense_category', 'br_ajax_delete_expense_category');
