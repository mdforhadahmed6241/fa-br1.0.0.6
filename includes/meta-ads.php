...
// NOTE: The menu registration function br_meta_ads_admin_submenu and its hook are removed
// because the menu is registered centrally in business-report.php.

function br_meta_ads_admin_enqueue_scripts( $hook ) {
    // Enqueue on Meta Ads page AND Settings page (for the new tab)
    if ( 'business-report_page_br-meta-ads' !== $hook && 'business-report_page_br-settings' !== $hook ) { 
        return; 
    }
...
/**
 * =================================================================================
 * 5. AUTO SYNC CRON FUNCTIONALITY (NEW)
 * =================================================================================
 */

/**
 * CRON JOB: Auto Sync Meta Ads Data (Runs twice daily)
 * * Automatically fetches and updates data for all active ad accounts.
 * Syncs the last 3 days (Today, Yesterday, Day Before) to ensure 
 * data attribution is up to date.
 */
function br_auto_sync_meta_ads_data() {
    $accounts = br_get_meta_accounts();
    if ( empty( $accounts ) ) {
        return;
    }

    // Sync range: Last 3 days (Today, Yesterday, Day Before Yesterday)
    // Going back 3 days helps capture delayed attribution data from Meta.
    ...
    foreach ( $accounts as $account ) {
        ...
        if ( $account->is_active ) {
            br_fetch_and_process_meta_data( $account, $start_date, $end_date );
        }
    }
}
    ...
    <div class="wrap br-wrap">
        ...
        <div class="br-page-content">
            ...
            switch($active_tab){
                ...
                // Settings case removed from here
        ...
        // Account modal moved to settings page logic
